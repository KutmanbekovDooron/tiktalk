...
class MessagesPresenter @Inject constructor(private val rxEventBus: RxEventBus) : BasePresenter<MessagesContract.View>(), ChatClientListener {
...
//                        chats.add(ChannelModel(channel, userData))
    ...
    fun deleteChat(name: String) {
        view?.let { view ->
            showDeleteChatDialog(view.getActivityContext(), name) {
//                TwilioSingleton.instance.chatClient.channels.getChannel()

                //CHa2d3e2ad712e4170a443d7ee1ab00e5f
                TwilioSingleton.instance.chatClient?.channels?.getChannel("CHa2d3e2ad712e4170a443d7ee1ab00e5f", object: CallbackListener<Channel>() {
                    override fun onSuccess(channel: Channel?) {
//                        val json = JSONObject()
//                        json.put("userId1", "s")
...
//                        })
                ...
                /*for (i in (0..chats.lastIndex).reversed()) {
                    ...
                    if (chats[i].isChecked) {
                        ...
                        TwilioSingleton.instance.chatClient.channels.getChannel(chats[i].sid, object: CallbackListener<Channel>() {
                            override fun onSuccess(channel: Channel?) {
                                Timber.e("Get channel = ${channel?.sid}, ${channel?.friendlyName}")
                                channel?.destroy(object: StatusListener() {
                                    override fun onSuccess() {
