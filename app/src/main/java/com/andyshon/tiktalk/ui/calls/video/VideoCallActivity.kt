package com.andyshon.tiktalk.ui.calls.video
...
import kotlinx.android.synthetic.main.activity_video_call_updated.*
import org.jetbrains.anko.alert
import org.jetbrains.anko.toast
import timber.log.Timber
import javax.inject.Inject

class VideoCallActivity : BaseInjectActivity(), CallsContract.View {
...
//            return sharedPreferences.getBoolean(SettingsActivity.PREF_ENABLE_AUTOMATIC_SUBSCRIPTION, SettingsActivity.PREF_ENABLE_AUTOMATIC_SUBCRIPTION_DEFAULT)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_video_call_updated)
        ...
        RxPermissions(this)
            .requestEach(Manifest.permission.RECORD_AUDIO, Manifest.permission.CAMERA, Manifest.permission.ACCESS_FINE_LOCATION)
            .subscribe ({ permission ->
                if (permission.granted) {
                    ...
                    videoCallRoot.post {
                        toast("Чтобы продолжить необходимо разрешить доступ к камере")
                    }


                    soundPoolManager?.stopRinging()
                    soundPoolManager?.playDisconnect()
                    videoCallRoot.postDelayed({
                        finish()
                    }, 350)
    ...
    private inner class VoiceBroadcastReceiver : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            Timber.e("VoiceBroadcastReceiver, onReceive")
    ...
    private fun handleIntent(intent: Intent?) {
        Timber.e("handleIntent ${intent?.action}")
        if (intent != null && intent.action != null) {
            when (intent.action) {
                ACTION_START_ACTIVITY -> {  // when user initiate call
                    ...
                    presenter.userPhoto = presenter.callToUser.photo
                ...
                ACTION_INCOMING_CALL -> {
                    ...
                    presenter.userPhoto = callInvite.callerAvatar
                    presenter.channelSid = callInvite.channelSid
...
//                            getUserLocation()
    ...
    private fun hideVideoTrack() {
        localVideoTrack?.let {
            val enable = it.isEnabled.not()
            it.enable(enable)
            if (enable) {
                it.addRenderer(thumbnailVideoView)
                callerAvatarBig.post {
                    thumbnailVideoView.show()
                    callerAvatarBig.hide()
                    btnHideVideoTrack.setColorFilter(ContextCompat.getColor(this, R.color.colorWhite), android.graphics.PorterDuff.Mode.SRC_IN)
                }
            }
            else {
                it.removeRenderer(thumbnailVideoView)
                callerAvatarBig.post {
                    thumbnailVideoView.hide()
                    callerAvatarBig.loadRoundCornersImageWithFallback(
                        url = presenter.userPhoto,
                        radius = getActivityContext().resources.getDimensionPixelSize(R.dimen.radius_0)
                    )
                    callerAvatarBig.show()
                    btnHideVideoTrack.setColorFilter(ContextCompat.getColor(this, R.color.colorBlack40), android.graphics.PorterDuff.Mode.SRC_IN)
                }
            }
        }
    }
    ...
    private fun rotateCameraClickListener(): View.OnClickListener {
    ...
    private fun muteClickListener(): View.OnClickListener {
        return View.OnClickListener {
            ...
            localAudioTrack?.let {
                ...
                it.enable(enable)
                if (enable)
                    btnMicrophoneOff.setImageResource(R.drawable.ic_mic_white_24dp)
                else
                    btnMicrophoneOff.setImageResource(R.drawable.ic_mic_white_off_24dp)
            }
        }
    }
    ...
    private fun setCommonCallListeners() {
        btnMicrophoneOff.setOnClickListener(muteClickListener())
        btnRotateCamera.setOnClickListener(rotateCameraClickListener())
        btnSendSms.setOnClickListener {}
        btnHideVideoTrack.setOnClickListener {
            hideVideoTrack()
        }
    }
...
//                presenter.sendStatusCallMessage(CallStatus.ENDED, chronometer.base)
    ...
    private fun setCallingUI() {
        ...
        callerAvatar.loadRoundCornersImageWithFallback(presenter.callToUser.photo)
...
//            disconnectFromChatRoom()
        ...
        callerAvatar.loadRoundCornersImageWithFallback(callInvite.callerAvatar)
