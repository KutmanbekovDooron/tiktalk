...
class SecretChatsPresenter @Inject constructor(
    ...
    fun setTwilioListener() {
        TwilioSingleton.instance.chatClient!!.addListener(this@SecretChatsPresenter)
    }
...
//                        chats.add(ChannelModel(channel, userData))
    ...
    fun deleteChat(name: String, secretChatSids: List<String>) {
        view?.let {
            showDeleteChatDialog(it.getActivityContext(), name) {
                secretChatSids.forEach { sid ->
                    secretChatsRepository.removeChatInSecrets(sid)
                }
                it.onChatDelete()
            }
        }
