...
class CameraCapturerCompat(context: Context, cameraSource: CameraCapturer.CameraSource) {
    ...
    private val camera2Listener = object : Camera2Capturer.Listener {
        override fun onFirstFrameAvailable() {
            Timber.e("$TAG, onFirstFrameAvailable")
        }

        override fun onCameraSwitched(newCameraId: String) {
            Timber.e("$TAG, onCameraSwitched: newCameraId = $newCameraId")
        }

        override fun onError(camera2CapturerException: Camera2Capturer.Exception) {
            Timber.e("$TAG, ${camera2CapturerException.message}")
        }
    }
    ...
    init {
        if (Camera2Capturer.isSupported(context)) {
            ...
            camera2Capturer = Camera2Capturer(context, getCameraId(cameraSource), camera2Listener)
        } else {
    ...
    fun switchCamera() {
        if (usingCamera1()) {
            ...
            val cameraSource = getCameraSource(camera2Capturer!!.cameraId)
